...
const { isAddress, getAddress, providers, Contract, ethers } = require('ethers');
...
async function encodeUSD(_networkId, _boss, _cost, _initdays = 0) {
	var _token = getNetwork(_networkId).USDT;

	return await encodeSubscription(_networkId, _boss, _token, _cost, _initdays);
}
...
async function getTransfers(startBlock, endBlock) {
	...
	for (let i = numBatches - 1; i >= 0; i--) {
		...
		console.log(`Processing batch ${i + 1}: Blocks ${startBatchBlock} to ${endBatchBlock}`);

		const events = await contract.queryFilter(filter, startBatchBlock, endBatchBlock);
